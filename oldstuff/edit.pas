...
begin
     ...
     if j=1 then znum:=zeilnum
        ...
        { this does not work with fpk pascal, so I take the alternativ code }
     ...
     { val(my_copy(textzeile,1,j-1),par[znum],error);}
     { this does not work with fpk for linux, so I use the following code }
     text_dummy:=copy(textzeile,1,j-1);
...
begin
     ...
     my_wwindow(52,3,78,24,'[HELP]','',true);
     ...
     my_wwindow(1,2,50,25,'[EDIT]','<ESC>',false);
     zeile:=1;spalte:=5;
     zeilnum:=1;
