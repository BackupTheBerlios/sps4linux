...
for i:= 1 to max_output_loop do begin
	wert:=0;
	for j:= 7 downto 0 do 
		if outputs[ouput_var[i]*8-j] then wert:=wert+npower[j];
...
{ Flag innerhalb der SPS auf high gestezt. dieses flag wird in einem sps programm ausgwertet. }
...
{ ganz normalen io eingangs wird per software abgefragt und bei psitiver flanke wird ein softwarezaehler heruntergezaehlt }
...
begin
	assign(F,IOFile);
	write('try to open the ports ...');
	reset(F,Sizeof(byte_value));
	write('ok, try to read...');
	seek(F,io_port);
